<form method="post" id="estateForm" class="was-validated" autocomplete="off">
        ...
        <div class="input-group col-sm-8">
            <select id="building_id" name="building_id" class="custom-select" required>
                ...
                @foreach(buildingsList() as $x)
                    <option @if($x->id == $estate->building_id) selected="selected"
    ...
    <div class="form-group row">
        <label for="old_id" class="col-2 col-form-label">{{ __('lang.old_number') }}</label>
        <div class="col-8">
        ...
        <div class="input-group col-sm-8">
            <select id="floor_id" name="floor_id" class="custom-select" required>
                ...
                @foreach(floorsList() as $x)
                    <option @if($x->id == $estate->floor_id) selected="selected"
                            @endif value="{{$x->id}}">{{$x->floor}}</option>
                @endforeach
            </select>
            <div class="input-group-append">
    ...
    <div class="form-group row">
        ...
        <div class="col-8">
            <input id="dimension" name="dimension" type="text" class="form-control" required autocomplete="off"
                   value={{ $estate->dimension }}>
        </div>
    </div>
    <div class="form-group row">
        <label for="telephone" class="col-2 col-form-label">{{ __('lang.tel') }}</label>
        <div class="col-8">
            <input id="telephone" name="telephone" type="text" data-placement="right" class="form-control"
                   autocomplete="off" value={{ $estate->telephone }}>
        </div>
    </div>
    <div class="form-group row">
